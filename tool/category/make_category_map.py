# -*- coding: utf-8 -*-
"""193개 긁기 단위 카테고리 배정표 — 단일 원본.

배정 대상은 시군구가 아니라 **긁기 단위**(`scratchUnitId`)다. 대부분은 통계청
시군구 코드 5자리와 같지만 **여덟은 아니다** — 통합으로 생긴 서울 `11000` ·
제주 `50000` · 부산 `26000` · 대구 `27000` · 인천 `28000` · 대전 `30000` ·
울산 `31000` 과 신설한 독도 `DK001`. **이 여덟의 코드와 이름은
`tool/map/merge_spec.py` 가 원본이고 이 파일이 그것을 import 한다** (2026-08-14).
예전에는 여기에 다시 적어 두어 두 곳이 어긋날 수 있었다.

광역시 다섯은 2026-08-20 에 더해졌다. 인천은 강화군·옹진군을 남겼다.
...
# 계획된 랜드마크 32개. `design/art-landmark-candidates.md` 와 같아야 한다.
...
#
# **2026-08-20 광역시 통합으로 37 → 32 가 됐다.** 통합 지역은 랜드마크를 하나만
# 가질 수 있어 대표를 남기고 나머지는 버렸다 (사용자 결정). 버린 다섯은
# 범어사·태종대(부산), 비슬산(대구), 대청호(대전), 대왕암(울산)이다.
# 강화 고인돌·백령도 두무진은 강화군·옹진군이 통합에서 빠져 그대로 남는다.
PLANNED_LANDMARKS = '''
11000 28710 28720 41115 41670 51210 51760 43720 43800 44760 44150
36110 52190 52800 12150 12210 47130 47170 48220 48890
50000 41610 41830 41820 48170 47940
26000 27000 28000 30000 31000
'''.split() + [DOKDO['code']]
...
# ── 인천광역시 3 ───────────────────────────────────────────────
# 2026-08-20 통합. 9개 구를 합치고 강화군·옹진군만 남겼다.
put(MERGE['인천광역시']['code'], 'city', '인천 도심 — 계양산 랜드마크의 폴백')
...
# ── 대전광역시 1 ───────────────────────────────────────────────
# 2026-08-20 통합. 유성온천 배정은 통합으로 사라졌다.
put(MERGE['대전광역시']['code'], 'city', '대전 도심 — 계룡산 랜드마크의 폴백')
...
# ── 대구광역시 1 ───────────────────────────────────────────────
# 2026-08-20 통합. 군위군까지 함께 합쳤다.
put(MERGE['대구광역시']['code'], 'city', '대구 도심 — 팔공산 랜드마크의 폴백')
...
# ── 부산광역시 1 ───────────────────────────────────────────────
# 2026-08-20 통합. 도심 6곳과 해안 7곳이 한 단위가 됐다. 해안 정점 비율이
# 0.657 로 높고 대표 랜드마크도 해운대라 바다로 둔다.
put(MERGE['부산광역시']['code'], 'sea', '부산 해안 — 해운대 랜드마크의 폴백')

# ── 울산광역시 1 ───────────────────────────────────────────────
# 2026-08-20 통합. 울주군까지 함께 합쳤다.
put(MERGE['울산광역시']['code'], 'city', '울산 도심 — 반구대 랜드마크의 폴백')
...
def _dart(codes):
    ...
    w('/// 서울 `11000` · 제주 `50000` 과 2026-08-20 광역시 통합으로 생긴\n')
    w('/// 부산 `26000` · 대구 `27000` · 인천 `28000` · 대전 `30000` ·\n')
    w('/// 울산 `31000` 은 합성 ID 이고, 독도 `DK001` 은 신설한 긁기 단위라\n')
    w('/// 통계청 네임스페이스 밖이다. 인천은 강화군 `28710` · 옹진군 `28720` 이\n')
    w('/// 통합에서 빠져 그대로 남는다.\n')
